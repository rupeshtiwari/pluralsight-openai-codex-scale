...
const GIT_BACKED = {
  ...
  'oncamera-markdown-lint-silent': 'it needs the real node_modules, so CHECK_ROOT cannot relocate it. Proven by hand both ways: it is what reported the 518 problems on plans/ExecPlan.md that put an error badge on the filmed tab, grouped by rule; with .markdownlint.json in place it goes green, and deleting that file turns it red again with the same MD060 count',
