...
const GIT_BACKED = {
  ...
  'oncamera-markdown-lint-silent': 'it needs the real node_modules and the bundled config schema, so CHECK_ROOT cannot relocate it. Its first version passed while the editor showed 590 errors, because it linted the pristine ExecPlan the preflight sees rather than the one Codex writes on camera. Rebuilt to three assertions and proven by hand on each: put a string-valued key in .markdownlint.json and it reports that an editor validating the config discards all of it; change the inline directive to the inert `<!-- markdownlint-disable -- why -->` form and it reports that suppression does not actually happen; drop MD013 and MD060 from the config and it reports that agent-shaped tables are rejected. All three go green together and only together',
