#!/usr/bin/env bash
# Preflight for one demo: m2-c6-recover-failed-automation
#
# Runs the checks that gate this clip -- those tagged [all], which gate every
# clip in Module 2, plus those tagged [c6] -- and writes a readable log to
# module2/logs/m2-c6_preflight.txt, grouped by the clip's four steps with each
# step mapped to the objective it serves. Full command output for the same run
# lands beside it as m2-c6_preflight.full.txt.
#
...
set -uo pipefail
exec "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/preflight_check.sh" c6
