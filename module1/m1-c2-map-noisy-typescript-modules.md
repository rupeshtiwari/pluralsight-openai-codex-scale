...
```bash
bash module1/scripts/m1-c2-map-noisy-typescript-modules.preflight.sh
```

It runs the checks tagged `[all]`, which gate every clip in the module, plus the ones tagged
`[c2]`, and must end `PASS: m1-c2 is ready.` If any check fails it names the check, why it
matters, and the command that fixes it. Do not record against a failing preflight.

`module1/scripts/preflight_check.sh c2` is the same run, if you prefer the argument form.
...
It writes `module1/logs/m1-c2_preflight.txt`: a one-page report grouped by this clip's four
