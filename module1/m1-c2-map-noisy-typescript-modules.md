...
```bash
module1/scripts/preflight_check.sh c2
```

It runs the checks tagged `[all]`, which gate every clip in the module, plus the ones tagged
`[c2]`, and rewrites this clip's transcript. It must end `PASS: m1-c2 is ready.` If any check
...
Either form writes `module1/logs/m1-c2_preflight.txt`: a one-page report grouped by this
...
beside it as `m1-c2_preflight.full.txt`.
