...
to the project name and the Local badge. It must read `demo/m1-c3-start`. On the run that found this it read
...
`demo/m1-c3-start`. Compare the whole path: the two folders differed only in a parent directory, so any
...
It must end `PASS: m1-c3 is ready.` Anything else names the check, why it matters, and the
command that fixes it — fix that before recording.

The report lands in `module1/logs/m1-c3_preflight.txt`, one page grouped by this clip's four
steps.
