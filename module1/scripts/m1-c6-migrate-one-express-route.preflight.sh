#!/usr/bin/env bash
# Preflight for one demo: m1-c6-migrate-one-express-route
#
# Runs the checks that gate this clip -- those tagged [all], which gate every
# clip in Module 1, plus those tagged [c6] -- and writes a readable log to
# module1/logs/m1-c6_preflight.txt, grouped by the clip's four steps with each
# step mapped to the objective it serves. Full command output for the same run
# lands beside it as m1-c6_preflight.full.txt.
#
