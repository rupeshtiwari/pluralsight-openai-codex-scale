...
`[c5]`, and must end `PASS: m1-c5 is ready.` If any check fails it names the check, why it
matters, and the command that fixes it. Do not record against a failing preflight.

`module1/scripts/preflight_check.sh c5` is the same run, if you prefer the argument form.
...
It writes `module1/logs/m1-c5_preflight.txt`: a one-page report grouped by this clip's four
