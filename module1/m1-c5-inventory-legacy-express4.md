...
```bash
module1/scripts/preflight_check.sh c5
```

It runs the checks tagged `[all]`, which gate every clip in the module, plus the ones tagged
`[c5]`, and rewrites this clip's transcript. It must end `PASS: m1-c5 is ready.` If any check
...
Either form writes `module1/logs/m1-c5_preflight.txt`: a one-page report grouped by this
...
beside it as `m1-c5_preflight.full.txt`.
